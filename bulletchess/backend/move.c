...
char * err_promotion_move_with(char origin_file, char origin_rank, char dest_file, char dest_rank) {
			if (dest_rank != '8' && dest_rank != '1'){
				return "Invalid promotion Move destination {destination}, must be a back rank";	
			}
          ...
          && (dest_file == origin_file 
			...
			return "Invalid promotion Move from {origin} to {destination}, not a legal Pawn move";
} 


char * err_generic_move_with(char origin_file, char origin_rank, char dest_file, char dest_rank) {
    if (dest_file == origin_file) { 
				return origin_rank != dest_rank 
								? 0 
								: "Invalid Move from and to {origin}, a Piece cannot "
								"move to the same Square it currently occupies";
		}
    ...
    return ((file_diff == 1 && rank_diff == 2) || (file_diff == 2 && rank_diff == 1))
						? 0 
						: "Invalid Move from {origin} to {destination}, illegal for every Piece"; 
}
...
char * error_from_move(move_t move) {
	switch(move.type) {
		...
		case PROMOTION_MOVE:
		return err_promotion_move(move.promotion);
		case NULL_MOVE:
		return 0;
		default:
